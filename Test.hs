                                                                                ...
                                                                                case inputList !! 0 of
                                                                                    ...
                                                                                    "change-password"   ->  do  users <- atomically $ readTVar userList
                                                                                                                ...
                                                                                                                atomically $ writeTVar userList (newUsers ++ [ User {   login = login user, 
                                                                                                                                                                        ...
                                                                                                                                                                        subscribe = subscribe user }])
                                                                                    "exam-schedule"     ->  do  doodles <- atomically $ readTVar doodleList
                                                                                                                schedules <- bestSchedule doodleList userList handle
...
printSchedule handle doodle = do    
    let (start, end) = slot $ maximumBy (compareSlotPreference) (slots doodle) 
...
bestSchedule doodleList userList handle = do 
    ...
    hPutStrLn handle $ show schedules
    return $ filter (checkUsersConfliction users) schedules
